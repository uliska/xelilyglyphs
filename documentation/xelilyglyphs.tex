...
\section*{Quick Start}
\xelilyglyphs{} enables \LaTeX{} to include arbitrary notational elements from the LilyPond%
\footnote{\url{http://www.lilypond.org}}
notation software like characters in continuous text.
As it uses \package{fontspec} to access glyphs from an OpenType font \xelilyglyphs{} works with \hologo{XeLaTeX} or \hologo{LuaLaTeX} exclusively.
...
\usepackage{xelilyglyphs} % Currently there are no package options available
\begin{document}
...
Apart from the Emmentaler glyphs \xelilyglyphs{} can include any notational construct that LilyPond can produce.
...
\xelilyglyphs{} came into existence when I looked for a way to include arbitrary notational elements in the continuous text of \LaTeX{} documents.
...
\xelilyglyphs{} accesses the Emmentaler font through the \package{fontspec} package and inserts its glyphs in the continuous text applying sophisticated control over scaling and spacing.
...
Each single element can be scaled and vertically shifted, but the settings can also be done per document to accomodate for unusual body fonts that might not match too well with \xelilyglyphs.
...
Details about extending \xelilyglyphs{} with additional commands can be found in \fref{chap:python_scripts}.
...
\footnote{\url{https://github.com/uliska/xelilyglyphs/issues}}
to get an idea where you could help.

\bigskip

Originally \xelilyglyphs{} was a project on its own but by now it is part of a rather large family of projects around engraving musical scores with LilyPond and typesetting texts with \LaTeX: openLilyLib%
...
\xelilyglyphs{} is developed in the \dir{xelilyglyphs} directory of this project site, there you can inspect the source, clone or fork the repository, and submit issue reports.
If you are interested in participating in the development of \xelilyglyphs, don't hesitate to contact us directly\footnote{\url{mailto:ul@openlilylib.org}}.

\section{Installation}
\label{sec:installation}

\paragraph{Installation Options}
There are several ways to get \xelilyglyphs{} up and running: As part of a \TeX{} distribution, as a download from CTAN%
\footnote{http://www.ctan.org/pkg/xelilyglyphs}
or through its development repository on GitHub%
\footnote{\url{https://github.com/uliska/xelilyglyphs}},
...
\xelilyglyphs{} is known to need fairly recent versions of \package{fontspec} and \hologo{LuaLaTeX}.
So if you haven't got \xelilyglyphs{} through your \LaTeX{} distribution you may have to take care that you use a sufficiently current distribution.
...
If you're familiar with Git the recommended way to use \xelilyglyphs{} is to create a fork of the GitHub repository (assuming you already have a Github account).
Inside the same \dir{tex/latex/} dir run \texttt{git clone git@github.com:YOURUSERNAME/xelilyglyphs.git} -- please don't provide an alternative destination name because the package should actually reside in a directory called \dir{xelilyglyphs/}.
Then \texttt{cd} into that directory and add the original repository with \texttt{git remote add upstream git@github.com:openlilylib/xelilyglyphs.git}.
From now on you will refer to your fork as \texttt{origin} and to the official repository as \texttt{upstream}.

The most immediate advantage of using \xelilyglyphs{} through the Git repository is that it is the easiest way to keep the package up to date.
...
If you install \xelilyglyphs{} manually you will have to make the included \emmentaler{} OpenType font available for \LaTeX.
Copy the complete \dir{fonts/} folder from the package directory to \dir{TEXMFHOME/fonts/opentype} (creating these subdirectories if they aren't already present on your system) and rename it to a more explicit name such as ‘emmentaler’ or ‘xelilyglyphs’.
An even better way would be not to copy the files but rather create a symlink to it with \texttt{cd TEXMFHOME/fonts/opentype} and \texttt{ln -s /path/to/xelilyglyphs/fonts emmentaler} \todo{check on Windows}.
That way no manual adjustments have to be made when \xelilyglyphs{} is updated and might bring new versions of the font files with it.\\
...
\xelilyglyphs{} is distributed under the \LaTeX{} Project Public License version 1.3 or (at your option) any later version of this license.
...
\medskip
\xelilyglyphs{} contains the \emmentaler{} OpenType font developed provided by the LilyPond project (\url{http://www.lilypond.org}).
...
As mentioned in the Quick Start at the beginning of this document you have to activate \xelilyglyphs{} with \cmd{usepackage\{fontspec\}} \cmd{usepackage\{xelilyglyphs\}}.
...
For these cases \xelilyglyphs{} provides four generic access commands:
\begin{itemize*}
\item \cmd{lilyGlyph}
...
While this command has originally been created to print images generated by LilyPond you can actually print \emph{any} image, e.\,g. scanned images from autographs, taking advantage of \xelilyglyphs' infrastructure, for example the optional arguments described in the following section.
...
This may for example be necessary if you use a text font which doesn't harmonize well with \xelilyglyphs' default settings, for example because of its unusual x-height.
...
\textit{Technically speaking \xelilyglyphs{} applies a fourth layer of scaling with image files.
...
However you are encouraged to create such a command the way we define predefined commands in \xelilyglyphs{} itself.
...
See our instructions on how to create predefined commands the \xelilyglyphs{} way in \fref{sec:howto_predefined_commands}.
...
\xelilyglyphs{} gives you the option to access the available font versions, but it may make more sense to appreciate them as “weights” -- although this is technically speaking or even conceptionally incorrect.
...
The optical size used by \xelilyglyphs{} defaults to 16.
...
A full list of available glyphs is available in the documentation folder of the \xelilyglyphs{} package or in LilyPond's original documentation at \url{http://www.lilypond.org/doc/v2.16/Documentation/notation/the-feta-font.html}.
...
Unfortunately they aren't present in \emmentaler{} because LilyPond draws them by itself, so \xelilyglyphs{} realizes them using included pdf image files.
...
\xelilyglyphs{} doesn't provide predefined commands as they can easily be entered as single strings to \cmd{lilyDynamics}, like \cmd{lilyDynamics\{sffzrmp\}}, resulting in \lilyDynamics{sffzrmp}.
...
If you simply want to add your own predefined commands “the \xelilyglyphs{} way” it is a good idea to read this section too, but you may also skip it and directly go to \ref{sec:directory_structure}.
...
\footnote{\url{https://github.com/openlilylib/xelilyglyphs}}.
If you have obtained \xelilyglyphs{} from there and want to write new commands on a regular basis, especially if you want to contribute your results back to the package, you will need to have a fair understanding of it.
...
If you have downloaded \xelilyglyphs{} from \textsc{ctan} there will be significant differences in the directory structure which are mentioned below, so you should read the section anyway.

If you are using \xelilyglyphs{} from a \TeX{} distribution you can't really consider the contents of this section because the files will be scattered over a number of system and distribution specific places which are out of our control.
...
contains the usual number of files such as \textsc{readme}, installation hints and licensing information as well as the main package file \texttt{xelilyglyphs.sty}.

\paragraph*{\dir{/core}
\footnote{For this section a leading “/” in directory names refer to the package root directory.}}
consists of include files for \texttt{xelilyglyphs.sty} that contain the fundamental program logic of the package.

\paragraph*{The \dir{/commands} directory}
contains files defining the predefined commands available to the end user of \xelilyglyphs.
...
The subdirectory \dir{xelilyglyphs\_logo} contains the logo of the package in \textsc{pdf} and \textsc{png} format along with its \texttt{.tex} source.
...
If you have obtained \xelilyglyphs{} any other way than through its Git repository you probably can't or don't want to modify the package files directly.
...
We have provided a skeleton directory structure in the file \dir{xelilyglyphs\_private.zip} which is located in the \dir{documentation/} directory of the package%
\footnote{Please consult the documentation of your \TeX{} distribution where to find these.
A good chance would be to run \texttt{kpsewhich xelilyglyphs.sty} which should find the location of the main package file}.
Extract this archive somehwere in your \LaTeX{} search path, e.\,g. to \dir{TEXMFHOME/tex/latex/xelilyglyphs\_private}.
...
There is some infrastructure in \xelilyglyphs{} (defined in the file dotted.inp) to faciliate dealing with dotted symbols, but this implementation isn't completely satisfactory so far.
...
\begin{itemize*}
\item If you only have the plain \xelilyglyphs{} package you can create new commands using Emmentaler glyphs.\\
...
\medskip
All Python scripts handle \texttt{-h/--help} and \texttt{-v/--version} arguments, printing a short usage help or the current \xelilyglyphs{} version.

\paragraph{Licensing Note}
All files generated by the Python scripts contain the \xelilyglyphs{} license preamble stating them to be licensed with the GPL.
...
\begin{itemize}
	...
	\xelilyglyphs{} prefixes commands that seem prone to ambiguity with “lily” followed by an uppercase letter: instead of \cmd{dynamics} we used \cmd{lilyDynamics}.
	...
	\begin{itemize}
		...
		[]You can even use this to create non-musical commands that print images and profit from \xelilyglyphs' infrastructure (like the automatic scaling with text size).]
...
The resulting file is a working \LaTeX{} document that uses the \xelilyglyphs{} package.
...
But of course we would be happy if you decided to contribute your commands as additions to the package -- we consider increasing coverage of glyphs through user contribution a natural way of evolution for \xelilyglyphs.
...
If you find you should create a new .inp file because your commands belong to a new category please take the existing files as a model and add an appropriate \cmd{input} statement to \texttt{xelilyglyphs.sty}.
For any newly created files copy\,\&\,paste the usual copyright comment at the beginning from another file.
\item \emph{Please} add documentation for your commands in the manual (\texttt{/documentation/xelilyglyphs.tex}).
...
Depending on the type of your \xelilyglyphs{} installation you may work either in the \dir{/glyphimages} directory of the package or in the \dir{xelilyglyphs\_private} directory you set up according to \fref{subsec:private_directory_structure}.
...
A command entry starts with the special key \verb|%%xelilyglyphs| on a single line.
...
This is a minimal working example of a \xelilyglyphs{} entry section:
...
This issue is on the \textsc{todo} list for one of the next releases of \xelilyglyphs.
...
The script tries to check if the input file is in the correct location -- which is considered true if it is in a \dir{defintions} folder and has a \dir{xelilyglyphs*} directory in its path.
...
In order to work the script expects the current working directory to be either the \dir{glyphimages} subdirectory of the package or the root of the \dir{xelilyglyphs\_private} directory.
...
\chapter{Contributing to \xelilyglyphs}
\label{chap:contributing}

The original motivation to create \xelilyglyphs{} was the need to include musical symbols in a critical report of a scholarly edition.
...
But \xelilyglyphs{} definitely is a package that lives on community contribution.
...
There are also existing issues with the \LaTeX{} implementation that can be seen in the project's issue tracker on GitHub (\url{https://github.com/openlilylib/xelilyglyphs}).
...
And finally I have some ideas how \xelilyglyphs' functionality could be enhanced, and others could come up with more ideas.
